...
snacks.setup({
  ...
  picker = {
    actions = vim.tbl_extend("force",
      ...
      {
        open_in_quicker = function(picker)
          ...
          for _, item in ipairs(items) do
            if item.file then
              ...
              if item.end_pos then
                qf_item.end_lnum = item.end_pos[1]
                qf_item.end_col = item.end_pos[2]
              end
...
-- Note: 'mode' handled by passing a table to map or multiple calls.
...
-- map("n", "<leader>sb", ...) -- Duplicate of Buffer Lines above, skipping
